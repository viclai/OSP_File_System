...
static ospfs_direntry_t *find_direntry(ospfs_inode_t *dir_oi, const char *name,
	int namelen);
...
{
	if (blockno * OSPFS_BLKSIZE < blockno)
		return (void*) NULL;
...
ospfs_inode_blockno(ospfs_inode_t *oi, uint32_t offset)
{
	...
	if (offset >= oi->oi_size || oi->oi_ftype == OSPFS_FTYPE_SYMLINK)
		...
		uint32_t *indirect_block = 
			ospfs_block(indirect2_block[blockoff / OSPFS_NINDIRECT]
			);
...
static int
ospfs_get_sb(struct file_system_type *fs_type, int flags, const char *dev_name,
	void *data, struct vfsmount *mount)
{
...
ospfs_dir_lookup(struct inode *dir, struct dentry *dentry, 
		struct nameidata *ignore)
{
	...
	for (entry_off = 0; entry_off < dir_oi->oi_size;
		...
		if (od->od_ino > 0 && strlen(od->od_name) == dentry->d_name.len
			...
			dentry->d_name.len) == 0) {
			entry_inode = ospfs_mk_linux_inode(dir->i_sb, 
				od->od_ino);
			if (!entry_inode)
...
allocate_block(void)
{
	...
	for ( ; index < ospfs_super->os_nblocks; index++) {
		if (bitvector_test(bitmap, index) == 1) {
...
free_block(uint32_t blockno)
{
	...
	// Check that the blockno isn't bogus here.
	if (blockno != 0)
...
add_block(ospfs_inode_t *oi)
{
	...
	if (n < OSPFS_NDIRECT && n >= 0) { // Add direct block
		dblock = allocate_block();
		if (dblock == 0) // Failed to allocate
			return -ENOSPC;
                clear_block(dblock);
                oi->oi_direct[n] = dblock;
        } else if (n < OSPFS_NDIRECT + OSPFS_NINDIRECT) { // Indirect
		if (oi->oi_indirect == 0) { // Create indirect block if needed
			allocated[0] = allocate_block();
			if (allocated[0] == 0) // Failed to allocate
				return -ENOSPC;

			clear_block(allocated[0]);
			oi->oi_indirect = allocated[0];
		}
		// Create the direct block
		dblock = allocate_block();
		if (dblock == 0) { // Failed
			// Free the indirect block if it was allocated
			if (allocated[0] != 0) {
		...
		clear_block(dblock);
		((uint32_t *) ospfs_block(oi->oi_indirect))[direct_index(n)] = 
			dblock;

	} else if (n < OSPFS_MAXFILEBLKS) { //Doubly indirect
		if (oi->oi_indirect2 == 0) {// Create indirect2 block if needed
			allocated[0] = allocate_block();
			if (allocated[0] == 0) // Failed to allocate
				return -ENOSPC;
		...
		indblock = ((uint32_t *) 
			ospfs_block(oi->oi_indirect2))[indir_index(n)];
		// Allocate the indirect block if needed
		if (indblock == 0) {
			allocated[1] = allocate_block();
			if (allocated[1] == 0) { // Failed to allocate
				// Free indirect2 block if allocated
				if (allocated[0] != 0) {
					free_block(allocated[0]);
					//oi->oi_indirect2 = 0;
				}
			...
			clear_block(allocated[1]);
			indblock = allocated[1];
		}
		// Create the direct block
		dblock = allocate_block();
		if (dblock == 0) { // Failed
			// Free the indirect2 block if it was allocated
                        if (allocated[0] != 0) {
				free_block(allocated[0]);
				oi->oi_indirect2 = 0;
			}
		...
		clear_block(dblock);
		((uint32_t *) ospfs_block(indblock))[direct_index(n)] = dblock;

	} else
		return -ENOSPC; // File is full

	oi->oi_size += OSPFS_BLKSIZE; // Update file size
	return 0;
}
...
remove_block(ospfs_inode_t *oi)
{
	...
	if (n < OSPFS_NDIRECT) { // Direct
		...
		// Get indirect block ptr
		if (oi->oi_indirect == 0)
			return -EIO;

		indblock = (uint32_t*) ospfs_block(oi->oi_indirect);
		if (indblock == 0)
			return -EIO;

		d_index = direct_index(n);
		// Free the direct block under the indirect block
		free_block(indblock[d_index]);
		indblock[d_index] = 0;

		// If we just removed ind block's only direct block,
		// free indirect block.
		if (d_index == 0) {
		...
		// Get indirect2 ptr and its last allocated indirect ptr
		int32_t ind_index = indir_index(n);
		int32_t d_index = direct_index(n);

		if (oi->oi_indirect2 == 0)
			return -EIO;

		indblock2 = (uint32_t*) ospfs_block(oi->oi_indirect2);
		indblock = (uint32_t*) ospfs_block(indblock2[ind_index]);
		...
		// free indirect block.
		if (d_index == 0) {
			free_block(indblock2[ind_index]);
			indblock2[ind_index] = 0;
			// If we just removed ind2 block's only indirect block,
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
	...
	int r = 0;
	int result; // Return value of add_block
	...
	while (ospfs_size2nblocks(oi->oi_size) < ospfs_size2nblocks(new_size)) {
		...
		if (result == -EIO)
			...
			while (ospfs_size2nblocks(oi->oi_size) > 
				ospfs_size2nblocks(old_size)) {
				result = remove_block(oi);
				if (result != 0)
					return result;
			}
			return result;
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		uint32_t notRead; // Number of bytes not read - copy_to_user
				  // returns this.
		uint32_t data_offset; // Offset from the start of the block
		uint32_t bytesLeft = count - amount; // Bytes left to copy
		...
		if (bytesLeft + data_offset > OSPFS_BLKSIZE)
			n = OSPFS_BLKSIZE - data_offset;
		else // Reached last block
			n = bytesLeft;

		notRead = copy_to_user(buffer, data + data_offset, n);
		if (notRead != 0) {
			retval = -EFAULT;
			goto done;
		}
	...
	loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		uint32_t n;
		char *data;
		uint32_t notWritten; // Number of bytes not written - 
				     // copy_from_user returns this.
		uint32_t data_offset; // Offset from the start of the block
		uint32_t bytesLeft = count - amount; // Bytes left to copy
		...
		if (bytesLeft + data_offset > OSPFS_BLKSIZE)
			n = OSPFS_BLKSIZE - data_offset;
		else // Reached last block to write
			n = bytesLeft;

		notWritten = copy_from_user(data + data_offset, buffer, n);
		if (notWritten != 0) {
			retval = -EFAULT;
			goto done;
		}
...
static int
ospfs_link(struct dentry *src_dentry, struct inode *dir, 
	...
	newDirEntry = create_blank_direntry(dir_oi);
	if (IS_ERR(newDirEntry))
		return PTR_ERR(newDirEntry);
	if (newDirEntry == NULL)
		return -EIO;

	/* Initialize new link information. */
	memcpy(newDirEntry->od_name, dst_dentry->d_name.name, 
		dst_dentry->d_name.len);
	newDirEntry->od_name[dst_dentry->d_name.len] = '\0';
	newDirEntry->od_ino = src_dentry->d_inode->i_ino;

	src_oi->oi_nlink++; // Increment link count
	return 0;
}
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	struct inode* i;
	ospfs_direntry_t* newDirEntry;
	ospfs_symlink_inode_t* symInode;
	...
	if (entry_ino == 0)
		return -ENOSPC;
	/* Initialize the directory entry for the symbolic link. */
	newDirEntry->od_ino = entry_ino;
	memcpy(newDirEntry->od_name, dentry->d_name.name, dentry->d_name.len);
	newDirEntry->od_name[dentry->d_name.len] = '\0';

	/* Initialize symbolic link inode structure. */
	symInode = (ospfs_symlink_inode_t*) ospfs_inode(entry_ino);
	if (symInode == 0)
		return -EIO;
	symInode->oi_nlink = 1;
	symInode->oi_ftype = OSPFS_FTYPE_SYMLINK;
	symInode->oi_size = strlen(symname);
	memcpy(symInode->oi_symlink, symname, symInode->oi_size);
	symInode->oi_symlink[symInode->oi_size] = '\0';
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	// Exercise: Your code here.
	char cond[5]; // Check to see if this is "root?"
	char* path;
	uint32_t pathSize;
	int isConditional;
	char* first; // symlink when the condition is true (root)
	char* second; // symlink when the condition is false (non-root)
	int i;

	path = (char*) oi->oi_symlink;
	pathSize = (uint32_t) strlen(path);
	strncpy(cond, path, 5);
	isConditional = (strcmp("root?", cond) == 0);
	if (isConditional) {
		first = path + 5;
		for (i = 5; i < pathSize; i++) {
			if (path[i] == ':')
				break;
		}
		first[i - 5] = '\0';
		...
		if (current->uid == 0) // root
			nd_set_link(nd, first);
		else
			nd_set_link(nd, second);
